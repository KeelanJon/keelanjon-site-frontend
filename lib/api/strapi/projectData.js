...
export async function getProjectBySlug(slug) {
  try {
    ...
    const res = await strapi.get(
      `/projects?filters[slug][$eq]=${slug}&populate[featuredImage][populate]=*&populate[blocks][on][shared.carousel][populate]=*&populate[blocks][on][shared.seo][populate]=*&populate[blocks][on][shared.rich-text][populate]=*&populate[blocks][on][shared.video][populate=*`
    )
